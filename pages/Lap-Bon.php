    ...
    <div class="box">
        ...
        <form method="post" enctype="multipart/form-data" name="form1" class="form-horizontal" id="form1">
            <div class="box-body">
                ...
                <div class="form-group">
                    <div class="col-sm-3">
                        <input name="order" type="text" class="form-control pull-right" id="order"
                            placeholder="No Order" value="<?php echo $Order; ?>" autocomplete="off" />
                    </div>
                    <!-- /.input group -->
                </div>
                <div class="form-group">
                    <div class="col-sm-3">
                        <input name="hanger" type="text" class="form-control pull-right" id="hanger"
                            placeholder="No Hanger" value="<?php echo $Hanger; ?>" autocomplete="off" />
                    </div>
                    <!-- /.input group -->
                </div>
            </div>
                    ...
                    <div class="box-body">
                        <table class="table table-bordered table-hover table-striped nowrap" id="example3"
                            ...
                            <tbody>
                                ...
                                $no = 1;
                                if ($Awal != "") {
                                    $Where = " WHERE CONVERT(VARCHAR, tgl_buat, 23) BETWEEN '$Awal' AND '$Akhir' ";
                                }
                                if ($Order != "") {
                                ...
                                // Jika tidak ada filter, gunakan filter tanggal default
                                if ($Awal == "" && $Order == "" && $Hanger == "") {
                                    $Where = " WHERE CONVERT(VARCHAR, tgl_buat, 23) BETWEEN '$Awal' AND '$Akhir' ";
                                }
                                $qry1 = sqlsrv_query($con, "SELECT * FROM db_dying.tbl_gantikain $Where ORDER BY id ASC");
                                while ($row1 = sqlsrv_fetch_array($qry1, SQLSRV_FETCH_ASSOC)) {
                                    $sqlgk = sqlsrv_query($con, " SELECT * FROM db_dying.tbl_bonkain WHERE id_nsp='$row1[id]' ORDER BY no_bon ASC");
                                    $rgk = sqlsrv_num_rows($sqlgk);
                                    ?>
                                    <tr bgcolor="<?php echo $bgcolor; ?>">
                                        <td align="center"><?php echo $no; ?></td>
                                        <td align="center">
                                            <div class="btn-group"><a
                                                    href="index1.php?p=input-bon-kain&id=<?php echo $row1['id']; ?>" class="btn btn-warning btn-xs <?php if ($_SESSION['akses'] == 'biasa') {
                                                           echo "disabled";
                                                       } ?>" target="_blank"><i class="fa fa-plus"></i> </a>
                                                <a href="EditKPE-<?php echo $row1['id']; ?>" class="btn btn-info btn-xs <?php if ($_SESSION['akses'] == 'biasa') {
                                                       echo "disabled";
                                                   } ?>" target="_blank"><i class="fa fa-edit"></i> </a>
                                                <a href="#" class="btn btn-danger btn-xs <?php if ($_SESSION['akses'] == 'biasa' or $rgk > 0) {
                                                    echo "disabled";
                                                } ?>" onclick="confirm_delete('index1.php?p=hapusdatagantikain&id=<?php echo $row1['id']; ?>');"><i
