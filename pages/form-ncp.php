...
$sqlCekNM = sqlsrv_query($con, "SELECT * FROM db_dying.tbl_ncp_memo WHERE jnsdata='NCP' AND nokk='$nokk' AND nokk_ncp='$nokkNCP' AND no_ncp='$NCPNO'", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
$cekNM = sqlsrv_num_rows($sqlCekNM);
$rcekNM = sqlsrv_fetch_array($sqlCekNM);
...
LEFT JOIN 
    db_dying.tbl_montemp b ON a.id = b.id_schedule
WHERE
    a.nokk = '$nokk' 
    AND NOT (a.no_mesin = 'CB11' OR a.no_mesin = 'WS11')
ORDER BY
    a.id DESC",
	array(),
...
$sqlCek1 = sqlsrv_query($con, "SELECT TOP 1
								c.*,a.id as ids,a.no_mesin, b.id as idm 
							FROM
								db_dying.tbl_schedule a
...
$qryLama = sqlsrv_query($con, "SELECT  STR(DATEDIFF(SECOND, GETDATE(), b.tgl_buat) / 3600) + ':' +
                                          RIGHT('0' + CAST((DATEDIFF(SECOND, GETDATE(), b.tgl_buat) % 3600) / 60 AS VARCHAR(2)), 2) AS lama FROM db_dying.tbl_schedule a
...
$sqlCek2 = sqlsrv_query($con, "SELECT
    ...
    CASE WHEN COUNT(lot) > 1 THEN '(' + CAST(COUNT(lot) AS VARCHAR) + 'kk)' ELSE '' END AS kk,
...
GROUP BY
	...
	id
ORDER BY
	id ASC", array(), array("Scrollable" => SQLSRV_CURSOR_STATIC));
...
$sqlCek3 = sqlsrv_query($con, "SELECT TOP 1 * FROM  db_dying.tbl_montemp WHERE id='$rcek1[idm]' AND (status='antri mesin' OR status='sedang jalan' OR status='selesai') ORDER BY id DESC ", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
$cek3 = sqlsrv_num_rows($sqlCek3);
$rcek3 = sqlsrv_fetch_array($sqlCek3);
$sqlCekAir = sqlsrv_query($con, "SELECT TOP 1 air_awal,waktu_tunggu FROM db_dying.tbl_montemp WHERE id='$rcek[idm]' ORDER BY id DESC", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
$cekAir = sqlsrv_num_rows($sqlCekAir);
$rcekAir = sqlsrv_fetch_array($sqlCekAir);
$sqlTopping = sqlsrv_query($con, "SELECT TOP 1 jml_topping FROM db_dying.tbl_hasilcelup WHERE nokk='$nokk' ORDER BY id DESC ");
$rTopping = sqlsrv_fetch_array($sqlTopping);
// $sqltarget = sqlsrv_query($con, "select if(b.tgl_target<=now(),'melebihi target','sedang berjalan') as cek_delay,a.target,(round(left(c.lama_proses,2))+round(right(c.lama_proses,2)/60,2)) as lama_proses,
...
$sqlCek4 = sqlsrv_query($con, "SELECT TOP 1 * FROM db_dying.tbl_montemp WHERE id='$rcek1[idm]' ORDER BY id DESC ");
$cek4 = sqlsrv_num_rows($sqlCek4);
$rcek4 = sqlsrv_fetch_array($sqlCek4);
$sqlNCP = sqlsrv_query($cond, "SELECT TOP 1 * FROM db_qc.tbl_ncp_qcf_now WHERE (nokk='$nokkNCP' OR nokk_salinan='$nokkNCP') AND dept='DYE' ORDER BY id DESC");
$cekNCP = sqlsrv_fetch_array($sqlNCP);
$sqlNCP1 = sqlsrv_query($cond, "SELECT TOP 1 * FROM db_qc.tbl_ncp_qcf_now WHERE (nokk='$nokk' OR nokk_salinan='$nokk') AND dept='DYE' ORDER BY id DESC");
$cekNCP1 = sqlsrv_fetch_array($sqlNCP1);
...
// jika tanggal buat ncp lebih kecil dari tanggal buat (hasil celup/proses), maka tanggal tsb tidak dipilih, dilipih ditanggal sebelumnya
$tgl_celup = sqlsrv_query($con, "SELECT TOP 1 * FROM db_dying.tbl_montemp WHERE nokk='00102495' AND tgl_buat < '$cekNCP[tgl_buat]' ORDER BY id DESC ");
$row_tgl_celup	= sqlsrv_fetch_array($tgl_celup);
		...
		<div class="box-body">
			<div class="col-md-6">
				<div class="form-group">
					<label for="no_po" class="col-sm-3 control-label">Manual <?php if ($Manual == "ya") {
																					echo "Selected";
																				}
																				echo $Manual; ?></label>
					<div class="col-sm-2">
						<select name="manual" id="manual" onChange="aktif(); window.location='?p=Form-NCP&manual='+this.value " class="form-control select2">
							...
							<option value="ya" <?php if ($_GET['manual'] == "ya") {
													echo "Selected";
												} ?>>Ya</option>
						</select>
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input name="nokk" type="text" class="form-control" id="nokk" onchange="window.location='?p=Form-NCP&manual=<?php echo $_GET['manual']; ?>&nokk='+this.value" value="<?php echo $_GET['nokk']; ?>" placeholder="No KK" required>
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input name="no_order" type="text" required class="form-control" id="no_order" placeholder="No Order" value="<?php
																																		if ($cek > 0) {
																																			echo $rcek['no_order'];
																																		} else {
																																			if ($r['NoOrder'] != "") {
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input name="po_rajut" type="text" class="form-control" id="po_rajut" placeholder="PO Rajut" value="<?php if ($nokkNCP != "") {
																																echo $cekNCP['po_rajut'];
																															} ?>" readonly="readonly">
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-2">
						<select name="k_resep" class="form-control" id="k_resep" required>
							<option value="">Pilih</option>
							<option value="-">-</option>
							<option value="0x" <?php if ($rcek1['k_resep'] == '0x') {
													echo "SELECTED";
												} ?>>0x</option>
							<option value="1x" <?php if ($rcek1['k_resep'] == '1x') {
													echo "SELECTED";
												} ?>>1x</option>
							<option value="2x" <?php if ($rcek1['k_resep'] == '2x') {
													echo "SELECTED";
												} ?>>2x</option>
							<option value="3x" <?php if ($rcek1['k_resep'] == '3x') {
													echo "SELECTED";
												} ?>>3x</option>
							<option value="4x" <?php if ($rcek1['k_resep'] == '4x') {
													echo "SELECTED";
												} ?>>4x</option>
							<option value="5x" <?php if ($rcek1['k_resep'] == '5x') {
													echo "SELECTED";
												} ?>>5x</option>
							<option value="6x" <?php if ($rcek1['k_resep'] == '6x') {
													echo "SELECTED";
												} ?>>6x</option>
							<option value="7x" <?php if ($rcek1['k_resep'] == '7x') {
													echo "SELECTED";
												} ?>>7x</option>
							<option value="8x" <?php if ($rcek1['k_resep'] == '8x') {
													echo "SELECTED";
												} ?>>8x</option>
							<option value="9x" <?php if ($rcek1['k_resep'] == '9x') {
													echo "SELECTED";
												} ?>>9x</option>
							<option value="10x" <?php if ($rcek1['k_resep'] == '10x') {
													echo "SELECTED";
												} ?>>10x</option>
							<option value=">10x" <?php if ($rcek1['k_resep'] == '>10x') {
														echo "SELECTED";
					...
					<label for="kategori_resep" class="col-sm-3 control-label">Kategori Resep</label>
					<div class="col-sm-2">
						<select name="kategori_resep" class="form-control" id="kategori_resep">
							<option value="">Pilih</option>
							<option value="Baru" <?php if ($rcek1['resep'] == 'Baru') {
														echo "SELECTED";
													} ?>>Baru</option>
							<option value="Lama" <?php if ($rcek1['resep'] == 'lama') {
														echo "SELECTED";
													} ?>>Lama</option>
						</select>
					</div>
			...
			<div class="col-md-6">
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input name="nokk_ncp" type="text" class="form-control" id="nokk_ncp" onchange="window.location='?p=Form-NCP&manual=<?php echo $_GET['manual']; ?>&nokk=<?php echo $_GET['nokk']; ?>&nokkncp='+this.value" value="<?php echo $_GET['nokkncp']; ?>">
					</div>
					<label for="ncp_hitung" class="col-sm-3 control-label">Hitung NCP</label>
					<div class="col-sm-3">
						<input name="ncp_hitung" class="form-control" value="<?php if ($nokkNCP != "") {
																					echo $cekNCP['ncp_hitung'];
																				} ?>" readonly>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<div class="input-group">
							<select class="form-control select2" name="no_ncp" id="no_ncp">
								...
								<?php
								if ($nokkNCP != "") {
									$Wncp = " (nokk='$_GET[nokkncp]' or nokk_salinan='$_GET[nokkncp]') and ";
								} else {
									$Wncp = " nokk='$_GET[nokkncp]' and ";
								}
								$qCek = sqlsrv_query($cond, "SELECT no_ncp_gabungan FROM db_qc.tbl_ncp_qcf_now WHERE $Wncp dept='DYE' ORDER BY id DESC");
					...
					<label for="test1_roll" class="col-sm-2 control-label">Test 1 Roll</label>
					<div class="col-sm-2">
						<select name="test1_roll" class="form-control" id="test1_roll">
							<option value="">Pilih</option>
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<input name="masalah_ncp" type="text" class="form-control" id="masalah_ncp" value="<?php if ($nokkNCP != "") {
																												echo $cekNCP['masalah'];
																											} ?>" readonly="readonly">
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-5">
						<div class="input-group">
							<select class="form-control select2" name="kategori_masalah" id="kategori_masalah">
								<option value="">Pilih</option>
								<?php
								$qCek = sqlsrv_query($con, "SELECT kategori_masalah FROM db_dying.tbl_kategori_masalah_ncp ORDER BY kategori_masalah ASC");
								while ($dCek = sqlsrv_fetch_array($qCek)) {
								?>
									<option value="<?php echo $dCek['kategori_masalah']; ?>" <?php if ($dCek['kategori_masalah'] == $rcekNM['kategori_penyelesaian']) {
																									echo "SELECTED";
																								} ?>><?php echo $dCek['kategori_masalah']; ?></option>
								<?php } ?>
							</select>
					...
					<label for="lot_ncp" class="col-sm-2 control-label">Lot NCP</label>
					<div class="col-sm-2">
						<input name="lot_ncp" type="text" required class="form-control" id="lot_ncp" placeholder="" value="<?php if ($nokkNCP != "") {
																																echo $cekNCP['lot'];
																															} ?>">
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input name="qty3" type="number" min="0" required class="form-control" id="qty3" placeholder="0.00" value="<?php echo $rcek4['rol']; ?>">
					</div>
					<label for="jml_ncp" class="col-sm-3 control-label">Roll NCP</label>
					<div class="col-sm-3">
						<input name="jml_ncp" type="number" min="0" required class="form-control" id="jml_ncp" placeholder="0.00" value="<?php if ($nokkNCP != "") {
																																				echo $cekNCP['rol'];
																																			} ?>" readonly="readonly">
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<div class="input-group">
							<input name="qty4" type="number" min="0" required class="form-control" id="qty4" placeholder="0.00" style="text-align: right;" value="<?php echo $row_tgl_celup['bruto']; ?>">
							<span class="input-group-addon">KGs</span>
						</div>
					</div>
					...
					<div class="col-sm-3">
						<div class="input-group">
							<input name="qty_ncp" type="number" min="0" required class="form-control" id="qty_ncp" placeholder="0.00" style="text-align: right;" value="<?php if ($nokkNCP != "") {
																																											echo $cekNCP['berat'];
																																										} ?>" readonly="readonly">
							<span class="input-group-addon">KGs</span>
						</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input name="no_mc" id="no_mc" class="form-control" value="<?php echo $rcek1['no_mesin']; ?>" <?php if ($_GET['manual'] != "ya") {
																															echo "readonly";
																														} ?>>
					</div>
					<?php
					$qMC = sqlsrv_query($con, "SELECT kode FROM db_dying.tbl_mesin WHERE no_mesin='$rcek1[no_mesin]'");
					$dMC = sqlsrv_fetch_array($qMC);
					?>
					<div class="col-sm-2">
						<input name="nama_mc" id="nama_mc" class="form-control" value="<?php echo $dMC['kode']; ?>" <?php if ($_GET['manual'] != "ya") {
																														echo "readonly";
																													} ?>>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input name="tglcelup" type="text" class="form-control" id="tglcelup" value="<?php echo substr($row_tgl_celup['tgl_buat'], 0, 10); ?>" readonly>
					</div>
					<label for="shift1" class="col-sm-1 control-label">Shift</label>
					<div class="col-sm-1">
						<input name="shift1" type="text" class="form-control" id="shift1" value="<?php echo $row_tgl_celup['g_shift']; ?>" readonly>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input name="no_program" type="text" class="form-control" id="no_program" value="<?php echo $row_tgl_celup['no_program']; ?>" readonly="readonly">
					</div>
					<label for="press_pump" class="col-sm-4 control-label">Press Pump</label>
					<div class="col-sm-3">
						<input name="press_pump" type="number" min="0" class="form-control" id="press_pump" value="<?php echo $row_tgl_celup['tekanan']; ?>" readonly="readonly">
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-2">
						<div class="input-group">
							<input name="loading" type="number" min="0" class="form-control" id="loading" value="<?php echo $rcek['loading']; ?>" readonly="readonly">
							<span class="input-group-addon">&#37;</span>
						</div>
					</div>
					<label for="nozzle" class="col-sm-4 control-label">Nozzle</label>
					<div class="col-sm-3">
						<input name="nozzle" type="number" min="0" class="form-control" id="nozzle" value="<?php echo $row_tgl_celup['nozzle']; ?>" readonly="readonly">
					</div>
				</div>
				<div class="form-group">
					<label for="l_r" class="col-sm-3 control-label">L:R</label>
					<div class="col-sm-3">
						<input name="l_r" type="text" class="form-control" id="l_r" value="<?php echo $row_tgl_celup['l_r']; ?>" readonly="readonly">
					</div>
					<label for="blower" class="col-sm-3 control-label">Blower</label>
					<div class="col-sm-3">
						<input name="blower" type="number" min="0" class="form-control" id="blower" value="<?php echo $row_tgl_celup['blower']; ?>" readonly="readonly">
					</div>
				</div>
				<div class="form-group">
					<label for="rpm" class="col-sm-3 control-label">RPM</label>
					<div class="col-sm-3">
						<input name="rpm" type="number" min="0" class="form-control" id="rpm" value="<?php echo $row_tgl_celup['rpm']; ?>" readonly="readonly">
					</div>
					<label for="plaiter" class="col-sm-3 control-label">Plaiter</label>
					<div class="col-sm-3">
						<input name="plaiter" type="text" class="form-control" id="plaiter" value="<?php echo $row_tgl_celup['plaiter']; ?>" readonly="readonly">
					</div>
				</div>
				<div class="form-group">
					<label for="cycle_time" class="col-sm-3 control-label">Cycle Time</label>
					<div class="col-sm-3">
						<input name="cycle_time" type="number" min="0" class="form-control" id="cycle_time" value="<?php echo $row_tgl_celup['cycle_time']; ?>" readonly="readonly">
					</div>
...
if ($_POST['simpan_kategori'] == "Simpan") {
	...
	$sqlData1 = sqlsrv_query($con, "INSERT INTO db_dying.tbl_kategori_masalah_ncp (kategori_masalah, tgl_buat) 
                                VALUES ('$kategoriM', GETDATE())");
	if ($sqlData1) {
		echo "<script>swal({
...
if ($_POST['save'] == "Simpan") {
	...
	$sqlSimpan = sqlsrv_query(
		$con,
		"INSERT INTO db_dying.tbl_ncp_memo (
								nokk,
								nokk_ncp,
								jnsdata,
								no_ncp,
								...
								" . (int)$_POST['rpm'] . ",   
								'$_POST[plaiter]',
								" . (int)$_POST['cycle_time'] . ",  
								'$_POST[po_rajut]',
								'$_POST[no_po]',
								'$_POST[no_hanger]',
								'$_POST[shift1]',
								" . (int)$_POST['ncp_hitung'] . ",   
								'$_POST[k_resep]',
	...
	if ($sqlSimpan) {
		$sqlDataQC = sqlsrv_query($cond, "UPDATE db_qc.tbl_ncp_qcf_now SET 
		  ncp_in_dye='1'
		  WHERE id='$_POST[id]'");
...
if (isset($_POST['update'])) {
	if ($_POST['update'] == "Ubah") {
		...
		$sqlUpdate = sqlsrv_query($con, "UPDATE db_dying.tbl_ncp_memo SET	
			...
			no_warna='$nowarna1',
			lot='$_POST[lot]',
			rol_induk=" . (int)$_POST['qty3'] . ",
			qty_induk=" . (int)$_POST['qty4'] . ",
			rol_ncp=" . (int)$_POST['jml_ncp'] . ",
			qty_ncp=" . (int)$_POST['qty_ncp'] . ",
			...
			press_pump=" . (int)$_POST['press_pump'] . ",
			loading=" . (int)$_POST['loading'] . ",
			nozzle=" . (int)$_POST['nozzle'] . ",
			l_r='$_POST[l_r]',
			blower=" . (int)$_POST['blower'] . ",
			rpm=" . (int)$_POST['rpm'] . ",
			plaiter='$_POST[plaiter]',
			cycle_time=" . (int)$_POST['cycle_time'] . ",
			po_rajut='$_POST[po_rajut]',
			po_number='$_POST[no_po]',
			no_hanger='$_POST[no_hanger]',
			shift='$_POST[shift1]',
			ncp_hitung=" . (int)$_POST['ncp_hitung'] . ",
			k_resep='$_POST[k_resep]',
			kategori_resep='$_POST[kategori_resep]',
			test1_roll='$_POST[test1_roll]',
			lot_ncp='$_POST[lot_ncp]',
			ket='$_POST[ket]',
			tgl_update=GETDATE()
