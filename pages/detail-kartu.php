...
session_start();
include("../koneksi.php");
$qCek = sqlsrv_query($con, "SELECT no_mesin,no_urut FROM db_dying.tbl_schedule WHERE id='$_GET[id]'");
      ...
      <table id="tbl2" class="table table-bordered table-hover table-striped" width="100%">
        ...
        <tbody>
          ...
          $qry = sqlsrv_query($con, " SELECT
                                      *,
                                      CASE
                                          WHEN DATEDIFF(DAY, tgl_delivery, GETDATE()) > 0 THEN 'Urgent'
                                          WHEN DATEDIFF(DAY, tgl_delivery, GETDATE()) > -4 THEN 'Potensi Delay'
                                          ELSE ''
                                      END AS sts
                                  FROM
                                      db_dying.tbl_schedule a
                                  WHERE
                                      a.no_mesin = '" . $rCek['no_mesin'] . "' 
                                      AND a.no_urut = '" . $rCek['no_urut'] . "' 
                                      AND NOT [status] = 'selesai'
          ...
          while ($rowd = sqlsrv_fetch_array($qry)) {
            ...
            $qCek = sqlsrv_query($con, "SELECT TOP 1 id AS idb FROM db_dying.tbl_montemp WHERE nokk='$rowd[nokk]'");
            $rCEk = sqlsrv_fetch_array($qCek);
          ?>
            <tr align="center">
              <td>
              ...
              <td>
                <?php echo $rowd['tgl_delivery']->format('Y-m-d'); ?>
              </td>
              ...
              <td>
                <div class="btn-group"><a href="#" id='<?php echo $rowd['id']; ?>' class="btn btn-xs btn-info schedule_edit1 <?php if ($_SESSION['lvl_id10'] == "3") {
                                                                                                                                echo "disabled";
                                                                                                                              } ?>"><i class="fa fa-edit"></i> </a><a href="#" onclick="confirm_del1('?p=sch_hapus&id=<?php echo $rowd['id'] ?>');" class="btn btn-xs btn-danger <?php if ($_SESSION['lvl_id10'] == "3" or $rCEk['idb'] != "") {
                                                                                                                                                                                                                                                                                    echo "disabled";
