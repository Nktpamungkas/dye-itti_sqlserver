...
$Dept = $_GET['dept'];
$Cancel = $_GET['cancel'];
$qTgl = sqlsrv_query($cond, "SELECT CONVERT(date,GETDATE()) AS tgl_skrg,CONVERT(date,GETDATE()) AS jam_skrg");
      ...
      <td>
        <table width="100%" border="1" class="table-list1">
          <tbody>
            ...
            $qry1 = sqlsrv_query($cond, "SELECT * FROM db_qc.tbl_ncp_qcf WHERE $Wdept CONVERT(date, tgl_buat) BETWEEN '$Awal' AND '$Akhir' $sts ORDER BY id ASC");
            while ($row1 = sqlsrv_fetch_array($qry1)) {
            ?>
              <tr valign="top">
                <td align="center"><?php echo $no; ?></td>
                <td align="center"><?php echo date("d/m/y", strtotime($row1['tgl_buat']->format('d/m/y'))); ?></td>
                ...
                <td align="center"><?php if ($row1['tgl_selesai'] != "") {
                                      echo date("d/m/y", strtotime($row1['tgl_selesai']->format('d/m/y')));
                                    } ?></td>
              </tr>
            <?php $no++;
