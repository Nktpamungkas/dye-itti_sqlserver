...
$qTgl = sqlsrv_query($con, "SELECT 
                                CONVERT(VARCHAR, GETDATE(), 23) AS tgl_skrg, 
                                CONVERT(VARCHAR, DATEADD(DAY, 1, GETDATE()), 23) AS tgl_besok");
...
<body>
    <table width="100%" border="0" class="table-list1">
        <thead>
            ...
            <tr valign="top">
                <td colspan="18"><strong>Periode: <?php echo $_GET['awal']; ?> s/d
                        <?php echo $_GET['akhir']; ?></strong><br />
      ...
      $sql = sqlsrv_query($con, "SELECT
                                    ...
                                    WHERE
                                        $shft
                                        CONVERT(VARCHAR, a.tgl_buat, 23) BETWEEN '$Awal' AND '$Akhir'
                                        AND a.status = 'OK' 
                                        AND (b.proses = 'Celup Greige' OR b.proses = 'Cuci Misty' OR b.proses = 'Cuci Yarn Dye (Y/D)')
                                    ORDER BY
                                        b.no_mesin ASC");
            ...
            <tr valign="top">
              ...
              echo "R.L";
                ...
                <td>
                    <div align="right">
                        <?php
              if (!empty($rowd['tgl_in'])) {
                // Format tgl_in as H:i
                echo $rowd['tgl_in']->format('H:i');
              }
              ?>
                    </div>
                </td>
                <td>
                    <div align="right">
                        <?php
              if (!empty($rowd['tgl_out'])) {
                // Format tgl_out as H:i
                echo $rowd['tgl_out']->format('H:i');
              }
              ?>
                    </div>
                </td>
                ...
                <td>
                    <div align="right">
                        <?php if ($rowd['mulai_stop'] != "") {
                echo date('H:i', strtotime($rowd['mulai_stop']));
              } ?>
                    </div>
                </td>
                <td>
                    <div align="right">
                        <?php if ($rowd['selesai_stop'] != "") {
                echo date('H:i', strtotime($rowd['selesai_stop']));
              } ?>
                    </div>
                </td>
                <td>
