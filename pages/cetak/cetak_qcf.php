...
// 	$r3 = sqlsrv_fetch_array($bng11);
		...
		<tr height="10 cm">
			...
			<td colspan="2">
				<pre>Tanggal	: <?php if ($rowsmp['tgl_buat'] == '') {
										if($rowsmp['tgl_update']!= NULL or $rowsmp['tgl_update']!=''){echo $rowsmp['tgl_update']->format('d-m-Y');}
									} else {
										if($rowsmp['tgl_buat']!=NULL or $rowsmp['tgl_buat']!=''){echo $rowsmp['tgl_buat']->format('d-m-Y');}
									} ?></pre>
			</td>
			<td colspan="2">
	...
	<?php
	if($rowsmp1['tgl_buat']!=NULL or $rowsmp1['tgl_buat']!=''){
		echo $rowsmp1['tgl_buat']->format('d-m-Y H:i:s');}else{echo'';}
	//} 
	?>
	<script>
		alert('cetak');
