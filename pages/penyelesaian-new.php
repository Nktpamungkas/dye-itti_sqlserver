...
$sqlCek = sqlsrv_query($cond, "SELECT TOP 1 * FROM db_qc.tbl_ncp_qcf_new WHERE id='$id' ORDER BY id DESC", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
$cek = sqlsrv_num_rows($sqlCek);
$rcek = sqlsrv_fetch_array($sqlCek);
$revisi_sebelumnya = $rcek['revisi'] - 1;
$sqlCek1 = sqlsrv_query($cond, "SELECT * FROM db_qc.tbl_ncp_qcf_new WHERE no_ncp='$rcek[no_ncp]' AND revisi='$revisi_sebelumnya' AND dept='DYE'", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
$dCek1 = sqlsrv_num_rows($sqlCek1);
$rcek1 = sqlsrv_fetch_array($sqlCek1);
$sqlNCPS = sqlsrv_query($con, "SELECT acc_perbaikan_dye FROM db_dying.tbl_ncp_memo WHERE no_ncp='$rcek1[no_ncp_gabungan]'");
$rNCPS = sqlsrv_fetch_array($sqlNCPS);
$sqlNCP = sqlsrv_query($con, "SELECT * FROM db_dying.tbl_ncp_memo WHERE id_ncp='$id'");
$rNCP = sqlsrv_fetch_array($sqlNCP);
?>
		...
		<div class="box-body">
			...
			<div class="col-md-12">
				<?php if (strtoupper($_SESSION['usrid']) == "ADM-DYE") { ?>
					...
					//$nokk=$_GET[nokk];
					$qry1 = sqlsrv_query($con, "SELECT TOP 1 a.no_mesin, b.g_shift, b.colorist FROM db_dying.tbl_schedule a LEFT JOIN
					db_dying.tbl_montemp b ON a.id=b.id_schedule WHERE a.nokk='$_POST[nokk]' AND a.proses='Celup Greige'
					ORDER BY a.id DESC", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));

					if ($qry1 === FALSE) {
						die(sqlsrv_errors());
					}
					$dtDye = sqlsrv_fetch_array($qry1);
					$cDye = sqlsrv_num_rows($qry1);

					$qryHC = sqlsrv_query($con, "SELECT TOP 1 * FROM db_dying.tbl_hasilcelup WHERE nokk='$_POST[nokk]' AND proses='Celup Greige' ORDER BY id DESC", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
					$dtHC = sqlsrv_fetch_array($qryHC);
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<div class="input-group date">
							...
							<input name="tgl_rencana" type="text" class="form-control pull-right" id="datepicker" placeholder="0000-00-00" value="<?php echo cek($rcek['tgl_rencana'], 'Y-m-d'); ?>" autocomplete="off" />
						</div>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-2">
						<div class="input-group date">
							<div class="input-group-addon"> <i class="fa fa-calendar"></i> </div>
							<input name="tgl_kembali_qcf" type="text" class="form-control pull-right" id="datepicker2" placeholder="0000-00-00" value="<?php echo cek($rcek['tgl_kembali_qcf'], 'Y-m-d'); ?>" autocomplete="off" />
						</div>
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<select class="form-control select2" multiple="multiple" data-placeholder="Jenis Masalah" name="rmp_benang[]" id="kerusakan">
							...
							$data = explode(",", $dtArr);
							$qCek1 = sqlsrv_query($cond, "SELECT nama FROM db_qc.tbl_masalah_ncp WHERE jenis='Penyelesaian' ORDER BY nama ASC");
							$i = 0;
						...
						<div class="col-sm-3">
							<div class="input-group">
								<select class="form-control select2" name="penanggung_jawab" id="penanggung_jawab">
									...
									while ($rtj = sqlsrv_fetch_array($qrytj)) {
									?>
					...
					<div class="col-sm-3">
						<div class="input-group">
							<select class="form-control select2" name="acc_perbaikan_dye" id="acc_perbaikan_dye">
								<option value="">Pilih</option>
								<?php
								$qCek = sqlsrv_query($con, "SELECT nama FROM db_dying.tbl_acc_perbaikan_dye  ORDER BY nama ASC");
								while ($dCek = sqlsrv_fetch_array($qCek)) {
								?>
									<option value="<?php echo $dCek['nama']; ?>" <?php if ($dCek['nama'] == $rNCP['acc_perbaikan_dye']) {
																						echo "SELECTED";
																					} ?>><?php echo $dCek['nama']; ?></option>
								<?php } ?>
							</select>
					...
					<div class="col-sm-3">
						<div class="input-group">
							<select class="form-control select2" name="perbaikan_dye" id="perbaikan_dye">
								<option value="">Pilih</option>
								<?php
								$qCek = sqlsrv_query($con, "SELECT perbaikan FROM db_dying.tbl_perbaikan_dye  ORDER BY perbaikan ASC");
								while ($dCek = sqlsrv_fetch_array($qCek)) {
								?>
									<option value="<?php echo $dCek['perbaikan']; ?>" <?php if ($dCek['perbaikan'] == $rNCP['perbaikan_dye']) {
																							echo "SELECTED";
																						} ?>><?php echo $dCek['perbaikan']; ?></option>
								<?php } ?>
							</select>
					...
					<div class="col-sm-3">
						<div class="input-group">
							<select class="form-control select2" name="kategori" id="kategori">
								<option value="">Pilih</option>
								<?php
								$qCek = sqlsrv_query($con, "SELECT kategori FROM db_Dying.tbl_kategori_penyelesaian  ORDER BY kategori ASC");
								while ($dCek = sqlsrv_fetch_array($qCek)) {
								?>
									<option value="<?php echo $dCek['kategori']; ?>" <?php if ($dCek['kategori'] == $rcek['kategori']) {
																							echo "SELECTED";
																						} ?>><?php echo $dCek['kategori']; ?></option>
								<?php } ?>
							</select>
...
if ($_POST['simpan_acc'] == "Simpan") {
	...
	$sqlData1 = sqlsrv_query($con, "INSERT INTO db_dying.tbl_acc_perbaikan_dye (nama, tgl_buat) VALUES ('$namaACC', GETDATE())");
	if ($sqlData1) {
		echo "<script>swal({
		...
		echo "<script>swal({
			...
			text: 'Klik Ok untuk coba kembali',
			type: 'error',
			}).then((result) => {
			if (result.value) {
					window.location.href='index1.php?p=penyelesaian-new&id=$id&awal=$_GET[awal]&akhir=$_GET[akhir]';
...
if ($_POST['simpan_perbaikan'] == "Simpan") {
	...
	$sqlData1 = sqlsrv_query($con, "INSERT INTO db_dying.tbl_perbaikan_dye (perbaikan, tgl_buat) VALUES ('$namaPerbaikan', GETDATE())");
	if ($sqlData1) {
		echo "<script>swal({
		title: 'Data Telah Tersimpan',   
		text: 'Klik Ok untuk input data kembali',
...
if ($_POST['simpan_kategori'] == "Simpan") {
	...
	$sqlData1 = sqlsrv_query($con, "INSERT INTO db_dying.tbl_kategori_penyelesaian (kategori, tgl_buat) VALUES ('$namaKategori', GETDATE())");
	if ($sqlData1) {
		echo "<script>swal({
		title: 'Data Telah Tersimpan',   
		text: 'Klik Ok untuk input data kembali',
...
if ($_POST['save'] == "Simpan") {
	...
	$sqlMc = sqlsrv_query($con, "SELECT kode FROM db_dying.tbl_mesin WHERE no_mesin='$rcek[mesin]'");
	...
	$sqlData = sqlsrv_query($cond, "UPDATE db_qc.tbl_ncp_qcf_new SET 
	      $tglRC
		  $tglKQC
		  $tglTrima
		  penyelesaian = '$kt1',
	...
	if ($sqlData) {
		...
		if ($_POST['tgl_kembali_qcf'] != "") {
			$tglKQC = " tgl_penyelesaian='$_POST[tgl_kembali_qcf]', ";
		} else {
			$tglKQC = " tgl_penyelesaian = null, ";
		}
		if ($_POST['tgl_rencana'] != "") {
			$tglRC = " tgl_rencana='$_POST[tgl_rencana]', ";
		} else {
		...
		$sqlDye = sqlsrv_query($con, "UPDATE db_dying.tbl_ncp_memo SET
		$tglRC
		$tglKQC
...
if ($_POST['simpan_penyelesaian'] == "Simpan") {
	$tAwal = $_GET['awal'];
	$tAkhir = $_GET['akhir'];
	$sqlData1 = sqlsrv_query($cond, "INSERT INTO tbl_masalah_ncp SET 
